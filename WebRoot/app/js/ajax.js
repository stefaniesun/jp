...
document.body.appendChild(load_element);
	...
	var httpUrl = "http://192.168.1.13:8080/Jp";
...
(function(w){
	...
	w.ajax_login = function(options){
		var data = getdata(options,'UserWS/loginOper.xyz');
		httpUrl=httpUrl+"UserWS/loginOper.xyz";
		console.log(httpUrl);
		mui.ajax(httpUrl,{
			data:data,
			dataType:'json',//服务器返回json格式数据
	...
	w.ajax_get_Marquee = function(options){
		...
		mui.ajax(httpUrl,{
			...
			error:function(xhr,type,errorThrown){
				console.log("发起请求："+httpUrl+"   "+errorThrown);
			}
		});
	}
	//获取推荐商品
